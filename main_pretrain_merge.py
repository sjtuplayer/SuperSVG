...
from models.merge_painter import Merge_model
from PIL import Image
from engine_pretrain_merge import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_merge',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_merge',
...
def main(args):
    ...
    # define the model
    model0=AttnPainterSVG(stroke_num=128,path_num=4,width=width)
    ckpt=torch.load('/home/huteng/SVG/AttnPainter/output-test/checkpoints.pt')
    model0.load_state_dict(ckpt,strict=False)
    model0.to(device)

    model=Merge_model(stroke_num=128,path_num=12,width=width)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, model0,data_loader_train,
