...
class AttnPainterSVG(nn.Module):
    ...
    def predict_path(self, x,num=128):
        strokes = self.encoder(x,num=num)

        return strokes

    def rendering(self, strokes, save_svg_path=None, result_queue=None, idx=None,**kwargs):
        imgs = []
    ...
    def rendering_with_list(self, strokes, save_svg_path=None, result_queue=None, idx=None):
        num_control_points = [2] * self.path_num
        shapes = []
        groups = []
    ...
    def loss(self, x, require_lpips=False):
        ...
        if self.control_num:
            ...
            random_num_strokes=random.randint(1,128)
            strokes = self.encoder(x,random_num_strokes)
        else:
            strokes = self.encoder(x)
        # strokes=strokes[:,64:,:]
        pred = self.rendering(strokes)[:, :3, :, :]
        pred.reshape(x.shape[0], -1, pred.shape[-2], pred.shape[-1])
        if not require_lpips:
            loss = F.mse_loss(pred, x)
            return loss
        else:
            loss_lpips = self.loss_fn_vgg(pred, x).mean() * 0.02
            loss_mse = F.mse_loss(pred, x).mean()
            loss = loss_lpips + loss_mse
            return loss, loss_mse, loss_lpips
    def loss_optimize(self, x, new_strokes,target_strokes,ori_strokes): #用于优化输入new_strokes的
        ...
        loss_stroke=self.dtw(new_strokes,target_strokes).mean()*0.001
        loss0 = F.mse_loss(x, pred)
        loss=loss0#+loss_stroke
        return loss,loss0,loss_stroke,pred
