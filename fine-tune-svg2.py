...
def main(args):
    ...
    for idx,file in enumerate(files):
        ...
        print(strokes.shape)
        # idx=torch.nonzero(strokes<0.01)
        # print(idx)
        # strokes[idx]=0
        ...
        exit()
        output = output[:, :3, :, :]
        loss=l2_loss(output, torch.ones_like(output).cuda()*-1)
        loss.backward()
        new_strokes=[]
        cnt=torch.zeros((block*block))
        for i in range(strokes.size(1)):
            if strokes.grad[0,i].abs().sum()>0.00001:
                new_strokes.append(strokes[0,i])
                cnt[i//model.stroke_num]+=1
            if (i+1)%model.stroke_num==0 and i//model.stroke_num!=block*block-1:
                cnt[i//model.stroke_num+1]=cnt[i//model.stroke_num]
        strokes=torch.stack(new_strokes,dim=0).detach().unsqueeze(0)
        stroke_ids=strokes.clone()
