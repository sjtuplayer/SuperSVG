...
def decode_by_id_map(image,model):
    ...
    segments = slic(image, n_segments=32, sigma=5, compactness=5) # 512*512, 90 segments
    ...
    print(strokes.size(0) * strokes.size(1)) # [90, 128, 27=24(参数点)+3(RGB)] 每个seg有128个stroke
    new_strokes = []
    # id_strokes=[]
    # cnt=1
    ...
    #         stroke[1:-3:2] = x1 + (x2 - x1) * stroke[1:-3:2] # 相对坐标改绝对坐标
    #         stroke[0:-3:2] = y1 + (y2 - y1) * stroke[0:-3:2]
    #         stroke[-3:] = cnt
    #         id_strokes.append(stroke)
    #         cnt+=1
    # id_strokes=torch.stack(id_strokes, dim=0).unsqueeze(0) # 90*128个stroke [1, 11520, 27] 最后三位为序号
    # id_map = model.rendering(id_strokes) # 图片对应位置为stroke的序号
    # id_map = id_map[0, 0, :, :] # [512, 512]
    # id=1
    # for i in range(len(imgs)):
    #     print(i)
    #     for j in range(len(strokes[i])):
    #         stroke = strokes[i][j]
    #         # if id in id_map or id+0.5 in id_map or id-0.5 in id_map:
    #         if id in id_map:
    #         #if True:
    #         #if id in ids:
    #             x1, x2, y1, y2 = coords[i]
...
###
    for i in range(len(imgs)):
        ...
        edge = edge[0]
        filtered_edge = [point for point in edge if point[0][0] != 0 and point[0][1] != 0] # array

        for j in range(1, len(strokes[i])): # 128
            ...
            output_tmp = id_map[:, :3, :, :]
            save_image(output_tmp, 'tmp-new0.jpg', normalize=False)
            ...
            if (id_map0 == j).sum() != 0 and (id_map1 == j).sum() != 0: # stroke经过边界,但不一定有控制点在边界外
                ...
                if p_cnt != 0:
                    possible_intersect = []
                    # stroke_x1 = stroke[1:-3:2].min()
                    for p in range(1, len(filtered_edge)):
                        if id_map[filtered_edge[p][0][0]][filtered_edge[p][0][1]]!=id_map[filtered_edge[p-1][0][0]][filtered_edge[p-1][0][1]]: # 可能的交点
                            # print(filtered_edge[p][0][0], filtered_edge[p][0][1])
                            possible_intersect.append(p)
                    # print(len(possible_intersect))
                    if len(possible_intersect)>1: 
                    # 找到所有可能的交点后取首尾两个
                        skip = int((possible_intersect[-1] - possible_intersect[0])/p_cnt)-1
                        if skip==0: skip=1
                        for q in range(possible_intersect[0], possible_intersect[-1]+1):
                            if q%skip == 0:
                                curr_stroke.append(filtered_edge[q][0][1]/w)
                                curr_stroke.append(filtered_edge[q][0][0]/h)
                        if len(curr_stroke)<24:
                            for t in range(int((24-len(curr_stroke))/2)):
                                curr_stroke.append(curr_stroke[-2])
                                curr_stroke.append(curr_stroke[-1])
                        curr_stroke = curr_stroke[0:24]
                        curr_stroke.append(stroke[-3].item())
                        curr_stroke.append(stroke[-2].item())
                        curr_stroke.append(stroke[-1].item())
                        stroke = torch.Tensor(curr_stroke)

            if (id_map1 == j).sum() == 0: continue
            
            output_tmp = model.rendering(stroke.unsqueeze(0).unsqueeze(0)) # 只渲染一条stroke
...
###
    ...
    save_image(output, 'tmp-new.jpg', normalize=False)
@torch.no_grad()
