...
from models.attn_painter_autoregression import AttnPainterSVG
from PIL import Image
from engine_pretrain_autoregression import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_autoregression',
...
def main(args):
    ...
    ckpt = torch.load('/home/huteng/SVG/AttnPainter/output_autoregression/checkpoints.pt')

    model.load_state_dict(ckpt,strict=True)
    # ckpt = torch.load('/home/huteng/SVG/AttnPainter/output-64/checkpoints-imagenet.pt')
    # keys = list(ckpt.keys())
    # for key in keys:
    #     new_key = key.replace('encoder', 'encoder.encoder1')
    #     ckpt[new_key] = ckpt[key]
    #     ckpt.pop(key)
    # model.load_state_dict(ckpt, strict=False)
    # critic=Wgan(None,input_dim=6)
    # critic.net.load_state_dict(torch.load('/home/huteng/SVG/AttnPainter/output_autoregression/wgan-checkpoints-backup.pt'))
    critic=None
    ...
    param_groups = optim_factory.param_groups_weight_decay(model_without_ddp.encoder.encoder2, args.weight_decay)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.save(model.state_dict(),args.output_dir+'/checkpoints%s-%d.pt'%(args.label_name,epoch))
